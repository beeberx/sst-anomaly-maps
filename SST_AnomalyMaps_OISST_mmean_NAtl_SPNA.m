...
for yy=1:length(selectyear)
    select3 = find(OISST_tvec(:,1)==selectyear(yy));
    OI_Anom_select = OI_Anom(:,:,select3);
    OI_NormAnom_select = OI_NormAnom(:,:,select3);
    OI_tvec_select = OISST_tvec(select3,:);
    ...
    close all;
    figure(1)
    for mm=1:length(select3)
        [tim_chck]=intersect(OISST_tvec(:,1:2),OI_tvec_select(:,1:2),'rows');
        if isempty(tim_chck);clear tim_chck;continue;else;clear tim_chck;end;

        subplot(3,4,OI_tvec_select(mm,2))
        ...
        if ~ismember(OI_tvec_select(mm,2),[9,10,11,12])
            set(ax,'xticklabel',[])
        end
        if ~ismember(OI_tvec_select(mm,2),[1,5,9])
            set(ax,'yticklabel',[])
        end
